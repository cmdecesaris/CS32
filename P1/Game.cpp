...
void Game::play()
{
	...
	//consider implementeing a pointer for Before class
	if (p == nullptr)
	{
		m_arena->display("");
		return;
	...
	do
	{
		...
		msg = "";
		cout << endl;
		cout << "Move (u/d/l/r/su/sd/sl/sr/c/q/b): "; //addtion of before class key word
		string action;
		getline(cin, action);
		if (action.size() == 0)
			p->stand();
		else
		{
			...
			case 'b':
				m_arena ->whatWasBefore().printWhatWasBefore();  //case declaration for before, the m_arena object can call the before function and from there print what was before
