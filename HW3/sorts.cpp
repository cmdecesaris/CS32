...
bool compareSensorPtr(const Sensor* lhs, const Sensor* rhs)
{
	...
	//I can call comparesensor and pass in the dereferenced POINTERS
	return compareSensor(*lhs, *rhs); //should work
...
void insertion_sort(vector<Sensor>& s, bool comp(const Sensor&, const Sensor&))
{
	...
	//MAKE this faster
	//if (s.size() == 2 && comp(s[1], s[0])) //only one part of sort
	//	swap(s[0], s[1]);
	...
	for (int i = 1; i < size; i++) {
		Sensor temp = s[i]; //store i's item
		int j = i;
		for (; j > 0; j--) {
			if (comp(s[j - 1], temp)) { 
				s[j] = temp;
				break; //reduces the amount of looping 
			}
			s[j] = s[j - 1];
		}
	}
...
int main()
{
	...
	// destruction time.
	{
		vector<Sensor> auxSensors(sensors); //vector of sensors
		vector <Sensor*> sensorPtrs;
		//loop them
		for (int k = 0; k < auxSensors.size(); k++) 
			sensorPtrs.push_back(&auxSensors[k]);//must push back addresses so pointers can be assigned
		
		sort(sensorPtrs.begin(), sensorPtrs.end(), compareSensorPtr);

		//loop again but now in right ordering
		for (int j = 0; j < sensorPtrs.size(); j++) 
			sensors[j] = *sensorPtrs[j];
		
		// DONE~~TODO:  Create a vector of Sensor pointers, and set each pointer
		//        to point to the corresponding Sensor in auxSensors.

		// DONE~~~TODO:  Sort the vector of pointers using the STL sort algorithm
		//        with compareSensorPtr as the ordering relationship.

		// DONE ~~~TODO:  Using the now-sorted vector of pointers, replace each Sensor
	...
	//random_shuffle(ids3.begin(), ids3.end()); // from
	//
	//	// Create a bunch of Sensors
	...
	//random_shuffle(ids4.begin(), ids4.end()); // from
	//
	//	// Create a bunch of Sensors
	...
	//vector<Sensor> sensors7a(unorderedSensors500);
	//timer2.start();
	//insertion_sort(sensors7a, compareSensor);
	//double elapsed7a = timer2.elapsed();
	//assert(isSorted(sensors7a));
	//vector<Sensor> sensors7b(unorderedSensors5000);
	//timer2.start();
	//insertion_sort(sensors7b, compareSensor);
	//double elapsed7b = timer2.elapsed();
	//assert(isSorted(sensors7b));
	//assert(elapsed7a * 10 < elapsed7b);
	//assert((elapsed7a * 7 * 15 )> elapsed7b);
}