...
#define  BBALL_H
#include <string>
typedef std::string SomeType; //turn in as a STRING type
...
public:
	...
	BballRoster(const BballRoster& bbr); //copy constructor 

	const BballRoster& operator=(const BballRoster& bbr);//assignment operator 

	~BballRoster() ; //distructor for bball
...
private:
	struct Player {
		...
		std::string m_lname;
		SomeType m_val;
		Player(std::string fname, std::string lname, SomeType val); //serves as a Player constructor
		Player* next;
		Player* prev;
	};
